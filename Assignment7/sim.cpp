...
int regint(string reg){
    ...
    if(reg == "at") return 1;
    if(reg[0] == 'v') return 2 + reg[1] - '0';
    if(reg[0] == 'a') return 4 + reg[1] - '0';
    if(reg[0] == 't' && reg[1] < '8') return 8 + reg[1] - '0';
    if(reg[0] == 't') return 24 + reg[1] - '8';
...
int main(int argc, char* argv[]){
    ...
    while(true){
        ...
        if(toks[0] == "add"){
            ...
            if(regs[reg1] != regs[reg2]) pc += offset;
        }
        else if(toks[0] == "beq"){
            cycles += cyc[7];
            int reg1 = regint(toks[1]);
            ...
            cout << "Unrecognised instruction, terminating run\n";
            return 0;
        }
    }
