...
int MEM(){
    ...
    if(stall_mode)
    {
        ...
        mem_wb.wb.isj = false;
        mem_wb.wb.isjr = false;
        mem_wb.wb.isjal = false;
...
int main(int argc, char* argv[]){
    ...
    //register initialisation for fact.txt
    // regs[regint("s1")] = 8;
    // regs[regint("s2")] = -8;
    // regs[regint("s0")] = -1;
    ...
    for(int i = 0; i <= i_ + 3; i++){
        ...
        if(hit_stall && cur_instr != empty_instruction)
        {
        ...
        {
            bool b = hit();
            if(cur_instr!=empty_instruction)
            {
                if(b)
                    cout << "hit" << endl;
                else
            ...
            if(!b && pen_cycle != 0 && cur_instr != empty_instruction) // to handle case when pen_cycle =  0
            {
                hit_stall = true;
                cur_instr = empty_instruction;
                cur_ins = "stall";
