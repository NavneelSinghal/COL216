...
void IF(){
    ...
    if(!do_fetch) return; //we don't want to fetch this instruction because right now we go through a stall 
    ...
    if(string_to_int(if_id.instruction) == 0){ //we signal to the control of this intruction that this is an empty instruction
        not_stall = false;
    }
    else{
        not_stall = true;
...
void EX(){
    ...
    if(id_ex.wb.isj || id_ex.wb.isjal){
        ...
        ex_mem.add_result = id_ex.pc + string_to_int(id_ex.sign_extend); //check if this int can be negative or not - done @akash
