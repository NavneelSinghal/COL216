...
int regint(string reg){ // regint takes in a register (like $t0 ot t0) and returns the integer corresponding to the register
    if(reg[0] == '$'){
        reg = reg.substr(1);
    }
...
int main(int argc, char* argv[]){
    ...
    while(true){
        ...
        if(toks[0] == "add"){
            ...
            cout << "reg[" << reg1 << "] becomes reg[" << reg2 << "] + reg[" << reg3 << "] which is " << old2 << " + " << old3 << " = " << regs[reg1] << endl;
        }
        else if(toks[0] == "sub"){
            cycles += cyc[1];
            int reg1 = regint(toks[1]);
            ...
            cout << "reg[" << reg1 << "] becomes reg[" << reg2 << "] << " << shift << " which is " << old2 << " << " << shift << " = " << regs[reg1] << endl;
        }
        else if(toks[0] == "srl"){
            cycles += cyc[3];
            int reg1 = regint(toks[1]);
            ...
            cout << "memory location at " << offset + regs[reg2] << " was changed to " << regs[reg1] << endl;
            ...
            cout << "data at location " << offset + regs[reg2] << " was stored at register number " << reg1 << endl;
        }
        else if(toks[0] == "bne"){
            cycles += cyc[6];
            int reg1 = regint(toks[1]);
            ...
            int reg1 = regint(toks[1]);
            pc = regs[reg1];
            cout << "Jump to register value instruction encountered, pc is now " << pc << endl;
            ...
            regs[regint("ra")] = pc;
            pc = stoi(toks[1]);
            cout << "Jump and link instruction encountered, pc is now " << pc << " and ra is now " << regs[regint("ra")] << endl;
            ...
            cout << "reg[" << reg1 << "] becomes reg[" << reg2 << "] + " << addend << " which is " << old2 << " + " << addend << " = " << regs[reg1] << endl;
        }
        else if(toks[0] == "END"){
            ...
            cout << "Unrecognised instruction " << toks[0] << ", terminating run\n";
            return 0;
        }
    ...
    cout << "Average instructions per cycle = " << ((float) execed / max(cycles, 1)) << "\n\n";
    cout << "Register file state at the end of execution:\n";
    REP(i, 32) cout << "register " << i << " = " << regs[i] << '\n';
    cout << '\n';
    cout << "Memory state at the end of execution (truncated to the first 20 elements):\n";
