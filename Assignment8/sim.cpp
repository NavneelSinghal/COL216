...
void EX() {
    ex_mem.add_result =
        id_ex.pc +
        string_to_int(
            id_ex.sign_extend);  // check if this int can be negative or not

    // TODO: find what alu should do, using the aluop and last 6 bits of
    // id_ex.sign_extend or something - work of the alu control unit

    // TODO: now compute the result in alu and assign ex_mem's corresponding
    // registers to the result (also see what zero is) - work of the alu
...
string instruction_to_32(string s) { return ""; }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < i_ + 3; i++) {
        ...
        // has been encountered the hazard type is stored in hazard_type hazard
        // handling starts
    }
}
